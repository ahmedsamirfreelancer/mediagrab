/**
 * Preload injected into the embedded Pinterest window. It runs in Pinterest's
 * page (DOM access) but in an isolated world with ipcRenderer, so it can:
 *   1. Show a small MediaGrab toolbar with the destination folder name.
 *   2. Drop a "⬇ تحميل" button on every pin (image OR video).
 *   3. Send the clicked pin's URL back to the main process, which forwards it
 *      to the MediaGrab window's normal download queue.
 *
 * This gives the user Pinterest's EXACT search results (real page, infinite
 * scroll) with one-click downloading — no scraping, no missing pins. The
 * server figures out per pin whether it's a video (yt-dlp) or an image
 * (resolved off the pin page), so both kinds download correctly.
 */
...
(function () {
  ...
  // Default folder = the search query (so a search's pins group together).
  let defaultFolder = '';
  ...
  // Paint a button according to whether its pin was already downloaded.
  ...
  // Queue every visible pin as ONE batch (so Stop can cancel them together).
  function downloadAllVisible() {
    ...
    if (urls.length) ipcRenderer.send('pinterest-embed:download', { urls, folder });
    return urls.length;
  }
  ...
  // Download only the pins whose checkbox is ticked.
  function downloadSelected() {
    const checked = Array.prototype.slice.call(document.querySelectorAll('.mg-sel:checked'));
    if (!checked.length) { alert('محدّدتش أي بِن. علّم على البنّات الأول (✓ في الركن).'); return 0; }
    if (!confirm('هتحمّل ' + checked.length + ' بِن محدد. متأكد؟')) return 0;
    const folder = currentFolder();
    const urls = [];
  ...
  async function resetMarks() {
    try { await ipcRenderer.invoke('pinterest-embed:clearDownloaded'); } catch {}
  ...
  async function refreshDownloadedMarks() {
    try {
      const ids = await ipcRenderer.invoke('pinterest-embed:downloadedIds');
  ...
  // True when el's nearest scrollable ancestor is the window/document rather
  // than an inner overflow:scroll container. A sticky element sticks relative
  // to its scroll container, so this tells us whose top:0 our toolbar must
  // compensate for — and whose we must leave alone.
  ...
  // Make room for our fixed toolbar (idempotent, runs every tick).
  ...
  function injectToolbar() {
    ...
    row1.innerHTML = '<strong style="color:#e60023;font-size:15px;">MediaGrab</strong><span style="opacity:.85;">📁 ينزّل في:</span>';
    ...
    openBtn.addEventListener('click', () => { ipcRenderer.invoke('pinterest-embed:openFolder', currentFolder()); });
    ...
    dlAll.title = 'ينزّل كل البنّات (صور/فيديو) الظاهرة دلوقتي';
    dlAll.style.cssText = btnStyle('#e60023');
    dlAll.addEventListener('click', () => {
      const count = document.querySelectorAll('.' + BTN_CLASS + '[data-vid]').length;
      if (!count) return;
      if (!confirm('هتحمّل كل الـ ' + count + ' بِن الظاهرين. متأكد؟')) return;
    ...
    // «تحديد» toggle — multi-select mode is off by default (clean grid).
    const selToggle = document.createElement('button');
    selToggle.id = 'mg-sel-toggle';
    selToggle.textContent = '☑ تحديد';
    selToggle.title = 'يفعّل وضع التحديد عشان تعلّم على كذا بِن وتحمّلهم مرة واحدة';
    ...
    selBtn.title = 'يحمّل البنّات اللي علّمت عليها (✓ في الركن)';
    ...
    stopBtn.addEventListener('click', async () => {
      try {
        const r = await ipcRenderer.invoke('pinterest-embed:stopAll');
    ...
    // Shown only while a single pin is open — downloads that exact pin.
    const curBtn = document.createElement('button');
    curBtn.id = 'mg-current-btn';
    curBtn.textContent = '⬇ حمّل البِن المفتوح';
    curBtn.title = 'يحمّل البِن اللي إنت فاتحه دلوقتي';
    curBtn.style.cssText = btnStyle('#16a34a') + 'display:none;';
    curBtn.addEventListener('click', () => {
      const m = location.pathname.match(/\/pin\/(\d+)/);
      if (!m) return;
      const url = 'https://www.pinterest.com/pin/' + m[1] + '/';
      ipcRenderer.send('pinterest-embed:download', { url, folder: currentFolder() });
      curBtn.textContent = '✓ في الطابور';
      setTimeout(() => { curBtn.textContent = '⬇ حمّل البِن المفتوح'; }, 2500);
    });
    ...
    ipcRenderer.invoke('pinterest-embed:baseDir').then((b) => { baseDir = b || ''; renderBasePath(); }).catch(() => {});
  }

  function addButtons() {
    // Only the search-results page gets grid buttons. When a pin is opened
    // Pinterest navigates to /pin/<id>/ whose related links would otherwise get
    // buttons sprayed across them — so bail unless we're on /search.
    ...
    for (const a of document.querySelectorAll('a[href*="/pin/"]')) {
      ...
      if (!m) continue;
      const id = m[1];
      // Skip the opened-pin dialog (related/comment links live there).
      if (a.closest('[role="dialog"]')) continue;
      // Grid cards only — real pin cards wrap an <img>; text links don't.
      if (!a.querySelector('img')) continue;
      // Exactly ONE button per pin — Pinterest renders several links per card.
      ...
      // z-index 50 (not max) so Pinterest's open-pin overlay paints OVER these.
      btn.style.cssText = 'position:absolute;top:8px;left:8px;z-index:50;background:#e60023;color:#fff;border:none;border-radius:8px;padding:6px 11px;font-size:12px;font-weight:700;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.5);';
      ...
      btn.addEventListener('click', (e) => {
        ...
        e.stopPropagation();
        const folder = currentFolder();
        ipcRenderer.send('pinterest-embed:download', { url, folder });
  ...
  function injectStyle() {
    ...
    // the per-pin «تحميل» button stays for one-click single downloads.
    s.textContent =
  ...
  function tick() {
    try {
      ...
      injectToolbar();
      pushPageDown();
      // When a pin is open the path leaves /search → hide grid buttons so they
      // never bleed over Pinterest's open-pin view.
      const onPin = /^\/pin\/\d+/.test(location.pathname);
      ...
      if (curBtn) curBtn.style.display = onPin ? '' : 'none';
      addButtons();
    } catch {}
  }
  ...
  // Pinterest loads pins lazily on scroll, so keep sweeping for new cards.
